...
int moveMaxToFront(ListNode **ptrHead){
    ...
    cur = (*ptrHead);
    maxItem = (*ptrHead)->num;
    //preMax gets to set the address of the ListNode right before the ListNode with the max value.
    ...
    //Move node with the max value to first position
    cur = preMax->next;
    preMax->next = cur->next;
...
int concatenate(ListNode **ptrHead1, ListNode *head2){ //Need to pass in address of Head1 just in case Head1 is NULL.
