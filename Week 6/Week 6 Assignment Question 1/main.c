...
Complex div(Complex *c1, Complex *c2)
{
    double denominator, temp, top_real, top_imag;
    Complex c3;
    denominator = (c2->real*c2->real)+(c2->imag*c2->imag);
    temp = c2->imag*-1;
    top_real = (c1->real*c2->real)-(c1->imag*temp);
    top_imag = (c1->real*temp)+(c2->real*c1->imag);
    c3.real = top_real/denominator;
    c3.imag = top_imag/denominator;
    return c3;
}
